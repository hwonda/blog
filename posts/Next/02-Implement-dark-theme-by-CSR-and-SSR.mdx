---
title: 우당탕탕 ServerComponent로 다크모드 구현하기(1)
desc: 당신의 Next.js는 SSR이 맞을까?
date: 2024-09-25
thumbnail: /posts/Next/02-Implement-dark-theme-by-CSR-and-SSR/thumbnail.gif
---

> 해당 글은 Next.js 14버전을 사용한 `Blog`를 기반으로 작성하였습니다.
...
<div style={{display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px'}}>
  <div>![](/posts/Next/02-Implement-dark-theme-by-CSR-and-SSR/1.png)</div>
  <div>![](/posts/Next/02-Implement-dark-theme-by-CSR-and-SSR/2.png)</div>
</div>
...
### 3. layout.tsx에 ThemeProvider 적용
...
### 4. client가 테마를 지정할 수 있도록 Header에 switch를 넣기
...
### 2. layout.tsx가 Client Component로 변질
`layout.tsx`는 **서버에서 렌더링**되어 **클라이언트로 전달**되는 **최상위 레이아웃**입니다.
하지만 `next-themes`를 사용하면서 `ThemeProvider`를 사용하게 되면, `layout.tsx`가 **클라이언트 컴포넌트**로 변질됩니다.
...
```
이렇게 `ThemeProvider`는 Client Component이기 때문에, 내부에 있는 컴포넌트들은 자동으로 Client Component로 변질됩니다.

이는 **서버에서 렌더링**되어 **클라이언트로 전달**되는 **최상위 레이아웃**의 의미를 퇴색시키는 문제를 야기합니다.

html, body 태그 외에는 모두 Client Component로 변질되어, **SEO에 불리**할 수 있습니다.
...
그러나 저의 경우에, **첫 페이지 로드 속도와 SEO**에 더 중점을 두고 있기 때문에, **CSR 방식보다는 SSR 방식**을 사용하는 것이 더 적합하다고 생각합니다.
