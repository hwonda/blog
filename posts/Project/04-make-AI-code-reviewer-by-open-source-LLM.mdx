...
코드래빗은 git의 변경점(`diff_text`)을 확인한 뒤 AI모델을 통해 코드를 리뷰해주고, 사용자와 대화형으로 피드백을 해주는 좋은 놈이었습니다.

그러나...
1. 클라우드 기반의 코드래빗이 회사 보안 정책상 사내 서버에 설치된 GitLab과 연동하기 어렵다.
2. 코드래빗이나 Claude, GPT API는 돈이 든다.
...
먼저 (사내)서버에 설치된 GitLab에 리뷰어 레포지토리(`mr-reviewer`)와 리뷰를 받을 레포지토리(`mmis`)를 준비합니다. 
그리고 Ollama를 같은 서버에 설치하고, API 엔드포인트를 확인합니다.

`mr-reviwer`에는 리뷰를 진행할 python 코드를 넣고, Ollama와 리뷰 받을 레포지토리를 연결합니다.
...
- deepseek-coder-33b : 한국어로 작성 시, 가끔 문법에 안 맞거나 한문이 섞여서 나오는 경우가 있음. 영어로 작성 시 가장 작성이 잘 됨.
...
이 외에도 llama, phi4, mistral 등 써봤는데, deepseek-coder-33b나 exaone3.5-32b가 가장 좋았습니다. 이후에는 deepseek로 결과값을 내고, exaone으로 한글번역을 해보려고 합니다. (나~중에)
